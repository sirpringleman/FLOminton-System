...
export default function App() {
  ...
  function handleAdminLogin(pwd) {
    if (!pwd) return;
    setAdminKey(pwd);              // treat any non-empty as valid
    try {
    ...
    <div className="app-shell">
                ...
                {notPresent.map((p) => (
                  ...
                  >
                    <span>
                      {p.name}{' '}
                      {isAdmin ? (
                        <span className="muted small-tag">
                          L{p.skill_level} · B{p.bench_count || 0}
                        </span>
                      ) : null}
                    </span>
                    {isAdmin && (
      ...
      {isDisplay && (
        <div className="display-overlay">
          ...
          <div className="display-courts display-two-by-two">
            {matches.map((m) => (
...
/* player chip now can show bench too */
...
/* Summary with real tab switch between Summary and System Diagnostics */
