/* logic.js
   ...
   Fairness (both modes):
   - Select players with fairness hard-guards (bench counts + last played)
   - Hard guard: no one benches twice before everyone benches once (early-night)
   - Tiered guard: strongly avoid benching players far above the minimum benches
   - Late-night bias: relax the “no double bench” rule so lucky players can be benched too
*/
...
// NEW: Path 1 tunables
const STRICT_GUARD_ROUNDS = 4;     // rounds where “no double bench before all bench once” is enforced
const BENCH_SPREAD_CAP    = 2;     // strongly avoid benching players more than min+2 benches
...
function safeSetLocal(k, v) { try { localStorage.setItem(k, currentMode); } catch {} }
...
export function selectPlayersForRound(present, roundNumber, lastRoundBenched = new Set(), courtsCount = 4) {
  ...
  // Fairness priority base ranking:
  //   Higher bench_count first (for "who should play"),
  //   then older last_played_round (those benched more recently get priority to play),
  //   then random tiebreak.
  ...
  // Bench distribution info
  const benchCounts = present.map((p) => p.bench_count || 0);
  const minBench   = Math.min(...benchCounts);
  const maxBench   = Math.max(...benchCounts);
  const benchCap   = minBench + BENCH_SPREAD_CAP;

  // Early / late night split
  const strictGuard = roundNumber <= STRICT_GUARD_ROUNDS;

  // Hard guard (early-night only):
  //   - strictGuard: no one benches twice before everyone benches once
  //   - late-night: allow everyone to be eligible so "lucky" players can be benched too
  ...
  // Tiered guard:
  //   strongly avoid benching players whose bench_count is far above the minimum
  const overCap = new Set(
    present
      .filter((p) => (p.bench_count || 0) > benchCap)
      .map((p) => p.id)
  );
  ...
  // Build bench from the "back" of the priority list (i.e. lowest bench_count first)
  const reversed = ranked.slice().reverse();
  const bench = [];

  const tryBench = (pred, allowOverCap = false) => {
    for (const p of reversed) {
      if (bench.length >= toBenchCount) break;
      if (bench.find((x) => x.id === p.id)) continue;

      if (!allowOverCap && overCap.has(p.id)) continue;
      if (!pred(p)) continue;
  ...
  // Phase 1: fill bench WITHOUT touching overCap players if at all possible
  // a) eligible & not consecutive
  tryBench((p) => benchEligible.has(p.id) && !avoidConsecutive.has(p.id), false);
  // b) eligible (even if consecutive)
  tryBench((p) => benchEligible.has(p.id), false);
  // c) not consecutive
  tryBench((p) => !avoidConsecutive.has(p.id), false);
  // d) anyone
  tryBench((_p) => true, false);

  // Phase 2: if still short, reluctantly allow overCap players
  if (bench.length < toBenchCount) {
    tryBench((p) => benchEligible.has(p.id) && !avoidConsecutive.has(p.id), true);
    if (bench.length < toBenchCount) tryBench((p) => benchEligible.has(p.id), true);
    if (bench.length < toBenchCount) tryBench((p) => !avoidConsecutive.has(p.id), true);
    if (bench.length < toBenchCount) tryBench((_p) => true, true);
  }

  // Try to swap to avoid consecutive benches where possible
  if (MAX_CONSECUTIVE_BENCH > 0) {
    for (let i = 0; i < bench.length; i++) {
      const p = bench[i];
      if (avoidConsecutive.has(p.id)) {
        const swap = ranked.find(
          (x) => !bench.find((b) => b.id === x.id) && !avoidConsecutive.has(x.id)
        );
