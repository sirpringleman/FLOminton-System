...
export default function App() {
    ...
    tickRef.current = setInterval(() => {
      setTimerLeft((prev) => {
        ...
        if (next <= 0) {
          clearTick();
          setRunning(false);
          // end-of-round → build quads, then transition
          beep(550, 600);
          (async () => {
  ...
  function handleAdminLogin(pwd) {
    if (!pwd) return;
    setAdminKey(pwd);          // unlock admin-only UI
    try {
       ...
       <div className="app-shell">
         ...
         {isSession && (
           <main className="session">
             {/* top row with timer + display button */}
         ...
         {isDisplay && (
           <div className="display-overlay">
             ...
             {/* force 2x2 layout via css class */}
         ...
         {/* SUMMARY / DIAGNOSTICS MODAL (body in part 3) */}
   ...
   function PlayerChip({ player, showSkill, showBench }) {
     return (
       <span className={`player-chip ${player.gender === 'F' ? 'f' : 'm'}`}>
         ...
         {showBench ? (
           <span className="skill-tag">B{player.bench_count ?? 0}</span>
         ) : null}
       </span>
     );
   }
...
function RundownModal({ onClose, payload }) {
  ...
  // merge players with session rows so those who never played still show
